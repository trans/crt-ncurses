...
describe CRT::Ncurses::Formatter do
  describe "plain text" do
    it "converts plain text to chtype array" do
      result, len, _ = CRT::Ncurses::Formatter.parse("Hello")
    ...
    it "returns empty array for empty string" do
      result, len, _ = CRT::Ncurses::Formatter.parse("")
      result.should be_empty
      len.should eq(0)
    end

    it "preserves character values" do
      result = CRT::Ncurses.fmt("ABC")
  ...
  describe "alignment" do
    it "defaults to left alignment" do
      _, _, align = CRT::Ncurses::Formatter.parse("text")
      align.should eq(CRT::Ncurses::LEFT)
    end

    it "detects [C] center alignment" do
      result, len, align = CRT::Ncurses::Formatter.parse("[C]centered")
    ...
    it "detects [center] alignment" do
      _, _, align = CRT::Ncurses::Formatter.parse("[center]text")
      align.should eq(CRT::Ncurses::CENTER)
    end

    it "detects [R] right alignment" do
      _, _, align = CRT::Ncurses::Formatter.parse("[R]right")
      align.should eq(CRT::Ncurses::RIGHT)
    end

    it "detects [right] alignment" do
      _, _, align = CRT::Ncurses::Formatter.parse("[right]text")
      align.should eq(CRT::Ncurses::RIGHT)
    end

    it "detects [L] left alignment" do
      _, _, align = CRT::Ncurses::Formatter.parse("[L]left")
      align.should eq(CRT::Ncurses::LEFT)
    end

    it "detects [left] alignment" do
      _, _, align = CRT::Ncurses::Formatter.parse("[left]text")
      align.should eq(CRT::Ncurses::LEFT)
    end

    it "only checks alignment at string start" do
      _, _, align = CRT::Ncurses::Formatter.parse("hello[C]world")
      align.should eq(CRT::Ncurses::LEFT)
  ...
  describe "attributes" do
    ...
    it "text before tag has no attributes" do
      result = CRT::Ncurses.fmt("plain[b]bold")
  ...
  describe "style stack" do
    ...
    it "pops last style with [/]" do
      result = CRT::Ncurses.fmt("[b]bold[/]plain")
    ...
    it "pops nested styles correctly" do
      result = CRT::Ncurses.fmt("[b]B[u]BU[/]B[/]plain")
      # 'B' — bold only
      attrs_of(result[0]).should eq(bold)
    ...
    it "resets all with [//]" do
      result = CRT::Ncurses.fmt("[b][u]styled[//]plain")
      attrs_of(result[0]).should eq(bold | underline)
  ...
  describe "escape sequences" do
    it "treats \\[ as literal bracket" do
      result, len, _ = CRT::Ncurses::Formatter.parse("a\\[b")
      len.should eq(3)
      text_of(result).should eq("a[b")
    end

    it "handles unclosed bracket as literal" do
      result = CRT::Ncurses.fmt("hello[world")
  ...
  describe "tabs" do
    ...
    it "expands tab after text to next boundary" do
      _, len, _ = CRT::Ncurses::Formatter.parse("ab\t")
  ...
  describe "color notation" do
    ...
    it "parses #XX as fg only" do
      fg, bg = CRT::Ncurses::Formatter.parse_color_token("#0F")
      fg.should eq(0x0F)
      bg.should eq(-1)
    end

    it "parses ##XX as bg only" do
      fg, bg = CRT::Ncurses::Formatter.parse_color_token("##B0")
      fg.should eq(-1)
      bg.should eq(0xB0)
    end

    it "parses #XX/YY as fg/bg" do
      fg, bg = CRT::Ncurses::Formatter.parse_color_token("#0F/B0")
      fg.should eq(0x0F)
      bg.should eq(0xB0)
    end

    it "parses ##YY/XX as bg/fg" do
      fg, bg = CRT::Ncurses::Formatter.parse_color_token("##B0/0F")
      fg.should eq(0x0F)
      bg.should eq(0xB0)
    end

    it "handles full range hex values" do
      fg, bg = CRT::Ncurses::Formatter.parse_color_token("#FF/00")
      fg.should eq(0xFF)
      bg.should eq(0x00)
    end
  end
  ...
  describe "named styles" do
    it "expands a registered style" do
      ...
      CRT::Ncurses.style("fancy", "bold u")
      result = CRT::Ncurses.fmt("[fancy]text[/]after")
      attrs_of(result[0]).should eq(bold | underline)
      attrs_of(result[4]).should eq(0)  # 'a' in "after"
    ensure
      CRT::Ncurses.styles.delete("fancy")
    end

    it "unknown single-token tag produces no attributes" do
      result = CRT::Ncurses.fmt("[unknown]text")
      attrs_of(result[0]).should eq(0)
      text_of(result).should eq("text")
    end
  end

  describe "CRT::Ncurses.fmt convenience" do
    it "returns chtype array directly" do
      result = CRT::Ncurses.fmt("hello")
      result.size.should eq(5)
      text_of(result).should eq("hello")
    end
  end
