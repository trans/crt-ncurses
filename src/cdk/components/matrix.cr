module CDK
  class Matrix < CDK::CDKObjs
    ...
    getter colvalues : Array(CDK::DisplayType)
    ...
    def initialize(cdkscreen : CDK::Screen, xplace : Int32, yplace : Int32,
                   ...
                   colwidths : Array(Int32), colvalues : Array(CDK::DisplayType),
      ...
      @colvalues = Array(CDK::DisplayType).new(cols + 1, CDK::DisplayType::MIXED)
      ...
      if @maxrt > 0
        @cell[0][0] = CDK.subwin(w, 3, @maxrt, begy, begx)
      end
      ...
      if have_coltitles
        (1..vcols).each do |x|
          cw = @colwidths[x]
          cell_width = cw + 3
          @cell[0][x] = CDK.subwin(w, borderw, cell_width, begy, begx)
      ...
      (1..vrows).each do |x|
        if have_rowtitles
          @cell[x][0] = CDK.subwin(w, 3, @maxrt, begy, xpos + borderw)
        ...
        (1..vcols).each do |y|
          ...
          @cell[x][y] = CDK.subwin(w, 3, cell_width, begy, begx)
      ...
      (1..cols).each do |y|
        @colvalues[y] = y < colvalues.size ? colvalues[y] : CDK::DisplayType::MIXED
      ...
      bind(:MATRIX, CDK::FORCHAR, :getc, LibNCurses::Key::PageDown.value)
      bind(:MATRIX, CDK::BACKCHAR, :getc, LibNCurses::Key::PageUp.value)
    ...
    def inject(input : Int32) : Int32
      ...
      if check_bind(:MATRIX, input)
        ...
        when LibNCurses::Key::Backspace.value, LibNCurses::Key::Delete.value
          if @colvalues[@col].viewonly? || @info[@row][@col].size <= 0
            ...
            if cell = cur_matrix_cell
              ...
              CDK::Screen.wrefresh(cell)
            end
            @info[@row][@col] = @info[@row][@col][0...charcount]
          end
        when LibNCurses::Key::Right.value, CDK::KEY_TAB
          if @ccol != @vcols
            @col += 1
            @ccol += 1
            moved_cell = true
        ...
        when CDK::COPY
          CDK::CDKObjs.paste_buffer = @info[@row][@col]
        when CDK::CUT
          CDK::CDKObjs.paste_buffer = @info[@row][@col]
          clean_cell(@row, @col)
          draw_cur_cell
        when CDK::ERASE
          clean_cell(@row, @col)
          draw_cur_cell
        when LibNCurses::Key::Enter.value, CDK::KEY_RETURN
        ...
        when CDK::REFRESH
          ...
          plainchar = CDK::Display.filter_by_display_type(@colvalues[@col], input)
          charcount = @info[@row][@col].size
          ...
          if plainchar == -1
            ...
            if cell = cur_matrix_cell
              ch = if CDK::Display.hidden_display_type?(@colvalues[@col])
                     @filler
                   else
                     plainchar
                   end
    ...
    def erase
      CDK.erase_curses_window(@cell[0][0]) if @maxrt > 0
      (1..@vrows).each { |x| CDK.erase_curses_window(@cell[x][0]) }
      (1..@vcols).each { |x| CDK.erase_curses_window(@cell[0][x]) }
      (1..@vrows).each do |x|
        (1..@vcols).each { |y| CDK.erase_curses_window(@cell[x][y]) }
      end
    ...
    def destroy
      clean_title
      CDK.delete_curses_window(@cell[0][0]) if @maxrt > 0
      (1..@vrows).each { |x| CDK.delete_curses_window(@cell[x][0]) }
      (1..@vcols).each { |x| CDK.delete_curses_window(@cell[0][x]) }
      (1..@vrows).each do |x|
        (1..@vcols).each { |y| CDK.delete_curses_window(@cell[x][y]) }
      end
    ...
    private def highlight_cell
      ...
      (1..@colwidths[@ccol]).each do |x|
        ch = if x <= infolen && !CDK::Display.hidden_display_type?(@colvalues[@col])
