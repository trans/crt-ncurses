module CDK
  class Mentry < CDK::CDKObjs
    ...
    def initialize(cdkscreen : CDK::Screen, xplace : Int32, yplace : Int32,
      ...
      field_rows = CDK.set_widget_dimension(parent_height, field_rows, 0)
      ...
      # Label window
      if @label.size > 0
        @label_win = CDK.subwin(w, field_rows, @label_len + 2,
          ypos + @title_lines + 1, xpos + horizontal_adjust + 1)
      end

      # Field window
      @field_win = CDK.subwin(w, field_rows, field_width,
    ...
    def inject(input : Int32) : String
      ...
      when LibNCurses::Key::Up.value
        ...
        CDK.beep if !moved && !redraw
      when LibNCurses::Key::Backspace.value, CDK::DELETE
        if @disp_type == CDK::DisplayType::VIEWONLY
          CDK.beep
        elsif @info.size == 0
          CDK.beep
        elsif input == CDK::DELETE
      ...
      when CDK::CUT
        if @info.size == 0
          ...
          CDK::CDKObjs.paste_buffer = @info.clone
          clean
          draw_field
        end
      when CDK::COPY
      ...
      when CDK::PASTE
        if CDK::CDKObjs.paste_buffer.size == 0
          CDK.beep
        else
      ...
      when CDK::REFRESH
        ...
        if @disp_type == CDK::DisplayType::VIEWONLY || @info.size >= @total_width
          CDK.beep
        else
