module CDK
  class Menu < CDK::CDKObjs
    ...
    def initialize(cdkscreen : CDK::Screen, menu_list : Array(Array(String)),
      ...
      (0...menu_items).each do |x|
        ...
        y1 = menu_pos == CDK::BOTTOM ? ymax - 1 : 0
        y2 = menu_pos == CDK::BOTTOM ? ymax - subsize[x] - 2 : TITLELINES
        ...
        x2 = if menu_location[x] == CDK::LEFT
               leftloc
             else
               rightloc -= (max + 2)
               rightloc
        ...
        @title_win[x1] = CDK.subwin(parent_window, TITLELINES,
          @menu_title_len[x1] + 2, ypos + y1, xpos + x2)
        @pull_win[x1] = CDK.subwin(parent_window, high, max + 2,
          ypos + y2, xpos + x2)
    ...
    def draw_subwin
      ...
      CDK::Screen.wrefresh(pw)

      # Highlight the title
      if tw = @title_win[@current_title]
        Draw.write_chtype_attrib(tw, 0, 0,
          @menu_title[@current_title], @title_attr, CDK::HORIZONTAL,
          0, @menu_title_len[@current_title])
        CDK::Screen.wrefresh(tw)
      end
    end

    def erase_subwin
      CDK.erase_curses_window(@pull_win[@current_title])
    ...
    def destroy
      (0...@menu_items).each do |x|
        CDK.delete_curses_window(@title_win[x])
        CDK.delete_curses_window(@pull_win[x])
      end
    ...
    private def draw_menu_title(item : Int32)
      ...
      Draw.write_chtype(tw, 0, 0, @menu_title[item],
        CDK::HORIZONTAL, 0, @menu_title_len[item])
      CDK::Screen.wrefresh(tw)
    end
