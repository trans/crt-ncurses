module CDK
  class Swindow < CDK::CDKObjs
    ...
    def initialize(cdkscreen : CDK::Screen, xplace : Int32, yplace : Int32,
                   ...
                   save_lines : Int32, box : Bool, shadow : Bool)
      super()
      ...
      @field_win = CDK.subwin(w, box_height - @title_lines - 2, box_width - 2,
      ...
      bind(:SWINDOW, CDK::BACKCHAR, :getc, LibNCurses::Key::PageUp.value)
      bind(:SWINDOW, 'b'.ord, :getc, LibNCurses::Key::PageUp.value)
      bind(:SWINDOW, 'B'.ord, :getc, LibNCurses::Key::PageUp.value)
      bind(:SWINDOW, CDK::FORCHAR, :getc, LibNCurses::Key::PageDown.value)
      ...
      (0...last_line).each do |x|
        ...
        if screen_pos >= 0
          Draw.write_chtype(fw, screen_pos, x,
            @list[idx], CDK::HORIZONTAL, 0,
            @list_len[idx])
        else
          Draw.write_chtype(fw, 0, x, @list[idx],
            CDK::HORIZONTAL, @left_char - @list_pos[idx],
            @list_len[idx])
        end
      end
