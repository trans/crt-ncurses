module CDK
  class Template < CDK::CDKObjs
    ...
    def initialize(cdkscreen : CDK::Screen, xplace : Int32, yplace : Int32,
                   title : String, label : String, plate : String,
                   overlay : String, box : Bool, shadow : Bool)
      super()
    ...
    def inject(input : Int32) : String
      ...
      when CDK::PASTE
        if CDK::CDKObjs.paste_buffer.size > 0
          clean
          CDK::CDKObjs.paste_buffer.each_char do |ch|
            handle_input(ch.ord)
          end
          draw_field
        else
    ...
    def handle_input(input : Int32)
      ...
      if input == LibNCurses::Key::Left.value
        ...
        if input == LibNCurses::Key::Backspace.value || input == CDK::DELETE
          if input == CDK::DELETE
            # Backspace behavior
            if mark != 0
        ...
        elsif CDK.is_char?(input) && @plate_pos < @plate.size
          # Insert character
          if mark < test.size
            chars = test.chars
    ...
    def draw_field
      ...
      if @overlay.size > 0
        Draw.write_chtype(fw, 0, 0, @overlay, CDK::HORIZONTAL, 0, @overlay_len)
      end
