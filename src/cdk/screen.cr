module CDK
  class Screen
    ...
    property object : Array(CDK::CDKObjs?) = Array(CDK::CDKObjs?).new(2, nil)
    ...
    def initialize(window : NCurses::Window)
      ...
      @object_count = 0
      @object_limit = 2
      @object = Array(CDK::CDKObjs?).new(@object_limit, nil)
      @window = window
      @object_focus = 0
    end

    # Register a CDK object with a screen
    def register(cdktype : Symbol, obj : CDK::CDKObjs)
    ...
    # Remove an object from the CDK screen
    def self.unregister(cdktype : Symbol, obj : CDK::CDKObjs)
    ...
    def set_screen_index(number : Int32, obj : CDK::CDKObjs)
    ...
    def self.raise_cdk_object(cdktype : Symbol, obj : CDK::CDKObjs)
      if obj.valid_obj_type?(cdktype)
    ...
    def self.lower_cdk_object(cdktype : Symbol, obj : CDK::CDKObjs)
      if obj.valid_obj_type?(cdktype)
        if screen = obj.screen
