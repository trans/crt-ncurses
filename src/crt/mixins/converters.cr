module CRT
  module Converters
    ...
    def char2chtype(string : String, to : Array(Int32), align : Array(Int32)) : Array(Int32)
      ...
      if string.size >= 3 && string[0] == CRT::L_MARKER
        if string[1] == 'C' && string[2] == CRT::R_MARKER
          align[align.size - 1] = CRT::CENTER
          start = 3
        elsif string[1] == 'R' && string[2] == CRT::R_MARKER
          align[align.size - 1] = CRT::RIGHT
          start = 3
        elsif string[1] == 'L' && string[2] == CRT::R_MARKER
          start = 3
        end
      end
      ...
      while from < string.size
        if !inside_marker
          if from + 1 < string.size && string[from] == CRT::L_MARKER &&
             (string[from + 1] == '/' || string[from + 1] == '!' || string[from + 1] == '#')
            inside_marker = true
          elsif from + 1 < string.size && string[from] == '\\' && string[from + 1] == CRT::L_MARKER
            from += 1
